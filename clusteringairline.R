...
View(airlinedata)
k <- kselection(airlinedata, parallel = TRUE, k_threshold = 0.9, )
k
clusplot(k)
?kselection
...
k <- kselection(airlinedata, parallel = TRUE, k_threshold = 0.9, max_centers=12)
k
...
xpm <- pam(xds, 2)
clusplot(xpm)
normalized_data <- scale(airlinedata[,2:12])
fit <- kmeans(normalized_data, 4) # 4 cluster solution
str(fit)
final2<- data.frame(airlinedata, fit$cluster) # append cluster membership
final2
final3 <- final2[,c(ncol(final2),1:(ncol(final2)-1))]
aggregate(airlinedata[,2:12], by=list(fit$cluster), FUN=mean)
